import utility
import shopping.shopping_cart

print(utility)  # <module 'utility' from 'C:\\Users\\javie\\Desktop\\programming-courses\\python-course\\python-modules\\utility.py'>

utility.divide(4, 2)
utility.multiply(2, 2)

print(shopping.shopping_cart)# <module 'shopping.shopping_card' from 'C:\\Users\\javie\\Desktop\\programming-courses\\python-course\\python-modules\\shopping\\shopping_card.py'>

shopping.shopping_cart.buy('apple')
